# Multi-Agents / Collaboration and Competition (Tennis)   
...
- [1.2 Tennis environment](#tennis_environment)
- [1.3 Solving the "Tennis" environment](#solving_environment)

[2. Required Python libraries](#libraries)  
[3. Solution implemented](#solution)  
[4. Script: `shared_actor_maddpg_with_team_reward.py`](#shared_actor_maddpg_with_team_reward)  
- [4.1 Overview](#shared_actor_maddpg_with_team_reward_overview)  
- [4.2 CLI arguments](#shared_actor_maddpg_with_team_reward_arguments)  
- [4.3 Logging](#shared_actor_maddpg_with_team_reward_logging)  
- [4.4 Plots](#plots)  
- [4.5 CSV summaries](#csv_summaries) 
- [4.6 JSON files](#json_files) 
- [4.7 Model checkpoints](#model_checkpoints) 
- [4.8 2-step runs (commands)](#shared_actor_maddpg_with_team_reward_runs)
...
In the "Tennis" environment, two agents control rackets to bounce a ball over a net.
...
<img src="pictures/tennis_game.gif" width="500"/>
...
See `report.pdf` for the methodology and results.
...
# 4. Script: `shared_actor_maddpg_with_team_reward.py`

<a id = 'shared_actor_maddpg_with_team_reward_overview'></a>
## 4.1 Overview

This script is an end-to-end training runner for a MADDPG agent on the Unity Tennis environment, handling a batch of 2 competing agents.
...
```bash
python shared_actor_maddpg_with_team_reward.py --cpu --search --max-trials 10 --seeds 29
```

Then, we run the script with the best hyperparameters as default hyper parameters across 10 seeds:
```bash
python shared_actor_maddpg_with_team_reward.py --cpu --seeds 33,41,66,39,8,77,21,20,44,22
```
