...
def _resolve_env_exe(arg_path: Optional[str]) -> str:
    """
    Resolve the path to the Unity Tennis executable.

    Strategy:
    - If --env-exe provided and exists, use it.
    - Else, check UNITY_TENNIS_EXE env var.
    - Else, search common local paths adjacent to this script.
    - Else, walk up to depth 3 and locate Tennis.exe.

    Raises:
        FileNotFoundError if no executable is found.

    Returns:
        str: Absolute path to Tennis.exe.
    """
    if arg_path and os.path.isfile(arg_path):
        return os.path.abspath(arg_path)
    env_path = os.getenv('UNITY_TENNIS_EXE')
    if env_path and os.path.isfile(env_path):
        return os.path.abspath(env_path)
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(here, 'Tennis_Windows_x86_64', 'Tennis.exe'),
        os.path.join(os.path.dirname(here), 'p3_collab-compet', 'Tennis_Windows_x86_64', 'Tennis.exe'),
        os.path.join(os.path.dirname(here), 'Tennis_Windows_x86_64', 'Tennis.exe'),
    ]
    for p in candidates:
        if os.path.isfile(p):
    ...
    for root, dirs, files in os.walk(base):
        ...
        if depth > 3:
            dirs[:] = []
            continue
        if 'Tennis.exe' in files:
            return os.path.abspath(os.path.join(root, 'Tennis.exe'))
    raise FileNotFoundError(
        "Unity Tennis executable not found. Pass --env-exe, set UNITY_TENNIS_EXE, "
        "or place Tennis.exe under Tennis_Windows_x86_64 adjacent to this script."
    )
...
def _default_env_settings() -> Dict[str, Any]:
    """
    Default environment/training stop conditions for Tennis (2 agents).
...
# ---- Unity vectorized env (2 agents) in a separate process to avoid hangs ----
def _unity_worker(worker_end, env_exe_path: str, seed: int, worker_id: int):
    ...
    Shapes:
    - reset(train_mode): returns observations of shape (2, 24)
    - step(actions): expects actions of shape (2, 2)
      returns obs (2,24), rewards (2,1) float32, dones (2,1) bool
    ...
    def _reset(train_mode: bool = True):
        ...
        return info.vector_observations  # (2, 24)

    def _step(actions: np.ndarray):
        info = env.step(actions)[brain_name]
        obs = info.vector_observations          # (2, 24)
        rewards = np.asarray(info.rewards, dtype=np.float32)  # (2,)
        dones = np.asarray(info.local_done, dtype=np.bool_)   # (2,)
        return obs, rewards[:, None], dones[:, None]          # (2,24), (2,1), (2,1)
...
class UnityVectorEnv:
    ...
    - The Unity ML-Agents binary can hang or crash the main process. Hosting it
    ...
    - A worker process executes _unity_worker(), which:
      ...
      * Handles 3 commands received over the Pipe:
          'reset' -> returns observations (2, 24)
          'step'  -> expects actions (2, 2), returns (obs, rewards, dones)
    ...
    def step(self, actions: np.ndarray):
        ...
        try:
            self.pipe.send(('step', {'actions': actions}))  # actions (2,2)
            obs, rewards, dones = self.pipe.recv()          # (2,24), (2,1), (2,1)
            infos = [{}] * obs.shape[0]
...
class FCQV(nn.Module):
    ...
    Not used in this MADDPG implementation.
    """
...
class CentralCritic2(nn.Module):
    """
    Centralized critic for 2-agent MADDPG with shared actor.
    Approximates per-agent Q-values given joint observation and joint action:
    Q(x, a) -> R^2, where x = [s1; s2], a = [a1; a2].
    """
    def __init__(self, obs_dim_total: int, act_dim_total: int, hidden_dims: Tuple[int,int]=(256,256), activation_fc=F.relu, device: Optional[torch.device]=None):
        super().__init__()
        self.activation_fc = activation_fc
        self.input_layer = nn.Linear(obs_dim_total, hidden_dims[0])
        self.hidden1 = nn.Linear(hidden_dims[0] + act_dim_total, hidden_dims[1])
        self.out_layer = nn.Linear(hidden_dims[1], 2)  # Q1, Q2
        self.device = GLOBAL_DEVICE if device is None else device
        self.to(self.device)

    def forward(self, obs_cat: torch.Tensor, act_cat: torch.Tensor) -> torch.Tensor:
        if not isinstance(obs_cat, torch.Tensor):
            obs_cat = torch.tensor(obs_cat, device=self.device, dtype=torch.float32)
        if not isinstance(act_cat, torch.Tensor):
            act_cat = torch.tensor(act_cat, device=self.device, dtype=torch.float32)
        x = self.activation_fc(self.input_layer(obs_cat))
        x = torch.cat([x, act_cat], dim=1)
        x = self.activation_fc(self.hidden1(x))
        return self.out_layer(x)  # (B,2)

    def load(self, experiences):
        states, actions, rewards, new_states, is_terminals = experiences
        states = torch.from_numpy(states).float().to(self.device)          # (B,48)
        actions = torch.from_numpy(actions).float().to(self.device)        # (B,4)
        new_states = torch.from_numpy(new_states).float().to(self.device)  # (B,48)
        rewards = torch.from_numpy(rewards).float().to(self.device)        # (B,2)
        is_terminals = torch.from_numpy(is_terminals).float().to(self.device)  # (B,1)
        return states, actions, rewards, new_states, is_terminals
...
class ReplayBuffer:
    """
    Joint experience replay buffer for 2-agent MADDPG with centralized critic.
    Stores one tuple per environment step: (x, a, r_vec, x', done)
    where x ∈ R48, a ∈ R4, r_vec ∈ R2, done ∈ {0,1}.
    """
    def __init__(self, max_size: int = 100000, batch_size: int = 500):
        self.ss_mem = np.empty(shape=(max_size), dtype=np.ndarray)  # (48,)
        self.as_mem = np.empty(shape=(max_size), dtype=np.ndarray)  # (4,)
        self.rs_mem = np.empty(shape=(max_size), dtype=np.ndarray)  # (2,)
        self.ps_mem = np.empty(shape=(max_size), dtype=np.ndarray)  # (48,)
        self.ds_mem = np.empty(shape=(max_size), dtype=np.ndarray)  # (1,)
        self.max_size = max_size
        self.batch_size = batch_size
        self._idx = 0
        self.size = 0
    def store(self, sample: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]) -> None:
        s_cat, a_cat, r_vec, p_cat, d = sample
        self.ss_mem[self._idx] = s_cat.astype(np.float32)
        self.as_mem[self._idx] = a_cat.astype(np.float32)
        self.rs_mem[self._idx] = r_vec.astype(np.float32)
        self.ps_mem[self._idx] = p_cat.astype(np.float32)
...
class DDPG:
    ...
    def _optimize(self, experiences) -> None:
        # Joint transitions: states (B,48), actions (B,4), rewards (B,2), next_states (B,48), is_terminals (B,1)
        states, actions, rewards, next_states, is_terminals = experiences

        # Target actions for both agents from shared target actor
        s1p, s2p = next_states[:, :24], next_states[:, 24:]
        a1p = self.target_policy_model(s1p)
        a2p = self.target_policy_model(s2p)
        a_cat_p = torch.cat([a1p, a2p], dim=1)  # (B,4)
        q_next = self.target_value_model(next_states, a_cat_p)  # (B,2)
        target_q = rewards + self.gamma * q_next * (1 - is_terminals)  # (B,2)

        # Critic loss: MSE over both agents' Qs
        q_pred = self.online_value_model(states, actions)  # (B,2)
        td_error = q_pred - target_q.detach()
        ...
        # Actor loss: average over agents, detaching the other agent's action in each term
        s1, s2 = states[:, :24], states[:, 24:]
        a1 = self.online_policy_model(s1)
        a2 = self.online_policy_model(s2)
        q1 = self.online_value_model(states, torch.cat([a1, a2.detach()], dim=1))[:, 0].mean()
        q2 = self.online_value_model(states, torch.cat([a1.detach(), a2], dim=1))[:, 1].mean()
        policy_loss = -(q1 + q2) * 0.5
    ...
    def train(self,
        ...
        # Centralized critic over joint obs/actions (2 agents)
        self.target_value_model = self.value_model_fn(2*nS, 2*nA)
        self.online_value_model = self.value_model_fn(2*nS, 2*nA)
        ...
        for episode in range(1, max_episodes + 1):
            ...
            agent_states = states  # current states for both agents
            agent_ep_returns = np.zeros((agent_states.shape[0],), dtype=np.float32)

            for _ in count():
                # Actions for both agents via shared actor + noise
                actions = np.vstack([self.training_strategy.select_action(self.online_policy_model, s) for s in agent_states])  # (2,2)
                obs, rewards, dones, _ = envs.step(actions)
                next_states = obs

                ep_env_steps += 1

                # Build joint sample once per env step
                s_cat = np.concatenate([agent_states[0], agent_states[1]], axis=0).astype(np.float32)  # (48,)
                a_cat = np.concatenate([actions[0], actions[1]], axis=0).astype(np.float32)            # (4,)
                # Raw environment rewards are per-agent (shape (2,)) and are used for reporting.
                r_vec_raw = rewards.squeeze(1).astype(np.float32)                                      # (2,)
                ns_cat = np.concatenate([next_states[0], next_states[1]], axis=0).astype(np.float32)   # (48,)
                d_any = float(np.any(dones))

                # Team reward for learning (shared training signal), with a mild terminal penalty.
                # Environment terminal occurs when either agent drops/out-of-bounds; only one agent gets -0.01.
                # We keep episode-level max cumulative return as a reporting metric (computed from r_vec_raw).
                team_lambda = 0.5
                r_team = float(np.sum(r_vec_raw) - team_lambda * d_any)
                r_vec_team = np.array([r_team, r_team], dtype=np.float32)                              # (2,)

                # Store shaped team reward in replay for learning.
                self.replay_buffer.store((s_cat, a_cat, r_vec_team, ns_cat, d_any))

                # Logging stats per agent
                agent_ep_returns += r_vec_raw
                ep_steps_sum += 1
            ...
            # Tennis score per episode = max over the 2 per-agent episodic return sums
            ep_max_sum_rewards_per_agent = tuple(float(x) for x in agent_ep_returns.tolist())
            ep_max_sum_rewards = float(max(ep_max_sum_rewards_per_agent))
            self.episode_reward.append(ep_max_sum_rewards)
            ...
            debug_message = (f'{self.log_prefix}elapsed_time {elapsed_str}, episode {episode-1:04}, '
                             ...
                             f'ep_max_sum_rewards_per_agent ({ep_max_sum_rewards_per_agent[0]:.6f},{ep_max_sum_rewards_per_agent[1]:.6f}), '
                             f'ep_max_sum_rewards {ep_max_sum_rewards:.6f}, '
    ...
    def evaluate(self, eval_policy_model: FCDP, envs: UnityVectorEnv, n_episodes: int = 1) -> Tuple[float, float]:
        ...
        Tennis metric: episode score is max over the two agents' episodic returns.
        """
        rs = []
        for _ in range(n_episodes):
            state = envs.reset(train_mode=True)  # (2,24)
            ...
            agent_returns = np.zeros((state.shape[0],), dtype=np.float32)
            for _ in count():
                actions = np.vstack([self.evaluation_strategy.select_action(eval_policy_model, s) for s in state])  # (2,2)
                obs, rewards, dones, _ = envs.step(actions)
                state = obs
                agent_returns += rewards.squeeze(axis=1).astype(np.float32)
                if np.any(dones):
                    break
            rs.append(float(np.max(agent_returns)))
...
def run_single_training(run_dir: str,
    ...
    # Centralized critic over joint dimensions (nS_total=2*nS, nA_total=2*nA)
    value_model_fn = lambda nS_total, nA_total: CentralCritic2(nS_total, nA_total, hidden_dims=tuple(hparams['hidden_dims']))
...
def main():
    ...
    Hypothesis tests only applies whene n multiple seeds are used  
    - Logging is reset per run to avoid duplicates.
    """
    ...
    parser.add_argument('--env-exe', type=str, default=None, help='Path to Unity Tennis executable.')
    parser.add_argument('--cpu', action='store_true', help='Force CPU (disable CUDA usage in PyTorch).')
    parser.add_argument('--max-grad-norm', type=float, default=0.0, help='Enable gradient clipping with this max L2 norm (<=0 disables).')
    args = parser.parse_args()

    global MAX_GRAD_NORM
    MAX_GRAD_NORM = float(args.max_grad_norm) if float(args.max_grad_norm) > 0.0 else None

    _set_global_device(force_cpu=bool(args.cpu))
    logging.info(f"Torch device: {GLOBAL_DEVICE}")
    logging.info(f"Grad clip max-norm: {MAX_GRAD_NORM if MAX_GRAD_NORM is not None else 'disabled'}")

    seeds = tuple(int(s) for s in args.seeds.split(',') if s.strip() != '')

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = os.path.join(RESULTS_DIR, 'collab_compet', f'run_{timestamp}')
    plots_dir = os.path.join(run_dir, 'plots')
    os.makedirs(plots_dir, exist_ok=True)
